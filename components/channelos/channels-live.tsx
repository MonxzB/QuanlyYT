...
function SortableHead({ column, label, activeKey, direction, onSort, className }: { column: SortKey; label: string; activeKey: SortKey | null; direction: SortDirection; onSort: (key: SortKey) => void; className?: string }) {
  ...
  return <TableHead className={className}><button type="button" onClick={() => onSort(column)} className="inline-flex h-8 max-w-full items-center gap-1.5 rounded-md px-1 font-medium hover:bg-slate-100 hover:text-slate-950" title={active && direction === "desc" ? "Bấm lần nữa để trở về thứ tự kéo thả" : `Sắp xếp theo ${label.toLowerCase()}`}><span className="truncate">{label}</span><Icon className={`size-3.5 shrink-0 ${active ? "text-indigo-600" : "text-slate-400"}`} /></button></TableHead>;
}
...
function InlineStatusSelect({ channel, canEdit, onSaved }: { channel: Channel; canEdit: boolean; onSaved: () => Promise<void> }) {
  ...
  return <div className="relative w-full min-w-0"><select aria-label={`Trạng thái ${channel.name}`} value={value} onChange={(event) => void changeStatus(event.target.value as ChannelStatus)} disabled={!canEdit || saving} className={`h-8 w-full rounded-full border-0 px-2 text-xs font-semibold outline-none ring-1 ring-inset disabled:cursor-default disabled:opacity-100 ${statusTone(value)}`}>{(Object.entries(statusLabels) as Array<[ChannelStatus, string]>).map(([status, label]) => <option key={status} value={status} className={statusTone(status)}>{label}</option>)}</select>{saving && <LoaderCircle className="pointer-events-none absolute right-1 top-2 size-4 animate-spin" />}</div>;
}
