...
export function DashboardLive({ data, onNavigate }: { data: DashboardData; onNavigate: (page: string) => void }) {
  const latestMetrics = data.metrics.at(-1);
  const summaryCards = [
    ["Tổng kênh", data.totalChannels.toLocaleString("vi-VN")],
    ["Lượt xem ghi nhận", compactNumber(latestMetrics?.views ?? 0)],
    ["Người đăng ký", compactNumber(latestMetrics?.subscribers ?? 0)],
    ["Cảnh báo mở", data.openAlertCount.toLocaleString("vi-VN")],
  ] as const;
  const statusCards = [
  ...
  return <div className="space-y-4">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{summaryCards.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-semibold text-slate-950">{value}</p></div>)}</section>
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">{statusCards.map(([label, value, helper, color]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-medium text-slate-500">{label}</p><span className="size-2 rounded-full" style={{ backgroundColor: color }} /></div><p className="text-[28px] font-semibold leading-none text-slate-950">{value}</p><p className="mt-2 text-xs text-slate-500">{helper}</p></div>)}</section>
